# TidyTuesdays

Repositório com minhas soluções para os desafios semanais do TidyTuesday, utilizando Python, Streamlit e técnicas de análise e visualização de dados.

## 📂 Projetos

| Data | Projeto | Descrição |
|------|---------|-----------|
| 21.04.26 | O Brasil gasta bem em saúde? | Dashboard sobre gastos em saúde do Brasil (2000 - 2023) baseado nos dados da OMS |
| 09.06.26 | Do Console para o Cinema | Dashboard sobre filmes baseados em videogames, com base em uma raspagem da Wikipedia | 

## 🛠️ Como Executar um Projeto

1. Clone o repositório:

```bash
git clone https://github.com/chosenduck/TidyTuesdays.git
cd TidyTuesdays
```

2. Crie e ative um ambiente virtual:

```bash
python -m venv .venv
.venv\Scripts\activate
```

3. Instale as dependências:

```bash
pip install -r requirements.txt
```

4. Navegue até o projeto desejado e execute:

```bash
cd global_health_spending
streamlit run app.py
```